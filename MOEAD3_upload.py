...
class Multi_task_individual(nn.Module):
    def __init__(self):
        ...
        self.conv_back2 = nn.Conv2d(64, dim_input, kernel_size=(3, 3), padding='same', stride=(1, 1))
        self.spe1 = nn.Conv1d(dim_input, 128, kernel_size=(1, 1), padding='same', stride=1)
        self.spe2 = nn.Conv1d(128, 256, kernel_size=(1, 1), padding='same', stride=1)
        self.spe2 = nn.Conv1d(256, dim_input, kernel_size=(1, 1), padding='same', stride=1)
        self.Residual1 = Residual_Block(32)
        self.Residual2 = Residual_Block(32)
        ...
        self.fc = nn.Linear(2592, dim_out_class)
        self.cross_loss = nn.CrossEntropyLoss()

        # self.conv_con_hf_1=nn.Conv2d(51, 96, kernel_size=(3, 3), padding='same', stride=1)
        # self.conv_con_hf_2=nn.Conv2d(96, 76, kernel_size=(3, 3), padding='same', stride=1)
        # self.conv_con_hf_3=nn.Conv2d(76, 134, kernel_size=(3, 3), padding='same', stride=1)
        # self.conv_con_hf_4=nn.Conv2d(134, dim_input, kernel_size=(3, 3), padding='same', stride=1)

        self.Octconv1 = OctConv(ch_in=24, ch_out=48, kernel_size=3, alphas=(0., 0.7))
        self.Octconv2 = OctConv(ch_in=48, ch_out=24, kernel_size=3, alphas=(0.7, 0.8))
        self.Octconv3 = OctConv(ch_in=24, ch_out=32, kernel_size=3, alphas=(0.8, 0))
        ...
        self.Residual1 = Residual_Block(32)
        self.Residual2 = Residual_Block(32)
        self.block1 = Residual_Block(32)
        self.block2 = Residual_Block(32)
        self.conv = nn.Conv2d(32, 128, kernel_size=(3, 3), padding='same', stride=1)
        self.conv2 = nn.Conv2d(128, 256, kernel_size=(3, 3), padding='same', stride=1)
        # self.conv3 = nn.Conv2d(128, 256, kernel_size=(3, 3), padding='same', stride=1)
        self.loss = torch.nn.L1Loss()

        self.pool = nn.MaxPool2d((2, 2), stride=2, padding=1)

        ###################分类
        self.class_redu = nn.Conv2d(103, 24, kernel_size=(3, 3), padding='same', stride=1)
        self.fc = nn.Linear(2592, 5)
        self.cross_loss = nn.CrossEntropyLoss()
    ...
    def forward(self, x_LRHS, x_PAN, x_class, gt_pan=None, gt_class=None):
        ...
        f_class = self.Residual1(self.a1 * pan_hf_1 + self.b1 * f_class)
        # f_class = self.pool(f_class)
        f_class = self.Residual2(self.a2 * f_class + self.b2 * f_class)
        f_class = self.pool(f_class)
        ...
        if gt_pan is None:
            ...
            # loss_mini = self.loss(mini_pan,gt_pan[:,:,46:72,46:72])
